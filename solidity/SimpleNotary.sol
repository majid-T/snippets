...
contract NotaryOffice {
    ...
    function storeDoc(string memory _docToSave) public payable {
        ...
        require(msg.value >= 1 ether,"Notary Office: We require 1 ether to save a document");
        emit Document_Stored(_docToSave,msg.sender);
        storHashInDocs(hashDoc(_docToSave));
    }
    
    
    // Public function to check for string if it has been saved
    function checkDocument(string memory _docToCheck) public payable returns(bool) {
        // require minimum 1 ether for each time checking to be paid
        require(msg.value>= 1 ether,"Notary Office: We require one ether for our services" );
    ...
    function checkout() public {
        require(msg.sender == officeOwner,"Notary Office:Only owner can checkout");
        require( address(this).balance> 1,"Notary Office:We still havent make that much money");
